...
export class ConfiguracoesEventoComponent {
  evento: Evento = { ...this.eventoService.evento() };
  salvo = false;

  constructor(private eventoService: EventoService) {
    effect(() => this.evento = { ...this.eventoService.evento() });
  }

  salvar() {
    this.eventoService.atualizar(this.evento).subscribe({
      next: atualizado => {
        this.evento = { ...atualizado };
        this.salvo = true;
        setTimeout(() => this.salvo = false, 1800);
      },
    });
  }
}
